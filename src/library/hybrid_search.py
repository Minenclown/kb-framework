...
from .search_providers import SearchResult as ProviderSearchResult, SemanticSearchProvider, KeywordSearchProvider
...
class HybridSearch:
    ...
    def _semantic_search(
        ...
        if self._semantic_provider is not None:
            try:
                provider_results = self._semantic_provider.search(query, limit)
    ...
    def _keyword_search_fts(
        ...
        if self._keyword_provider is not None:
            try:
                provider_results = self._keyword_provider.search(query, limit)
                # Convert SearchResult to dict for compatibility with _merge_and_rank
                return [
                    {
                        "section_id": r.section_id,
